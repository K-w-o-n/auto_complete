...
autoCompleteInputTag.addEventListener("keyup", (e) => {
  ...
  resultContainerTag.innerHTML = ""; //delete all child element
...
const navigateAndSelect = (key) => {
  if (key === "ArrowDown") {
    ...
    indexToSelect += 1;
    const selectProduct = selectedClass(indexToSelect);
    if (indexToSelect > 0) {
      deSelected();
    }
    selectProduct.classList.add("selected");
  } else if (key === "ArrowUp") {
    ...
    const enterProduct = selectedClass(indexToSelect);
    console.log(enterProduct);
  }
};

const deSelected = () => {
        const selectedProduct = document.getElementsByClassName("selected")[0];
...
const selectedClass = (index) => {
      ...
      const selectProduct = document.getElementById(productIdToSelect);
      selectProduct.style.backgroundColor = "#237BFF";
      selectProduct.firstChild.style.color = "#fff";
